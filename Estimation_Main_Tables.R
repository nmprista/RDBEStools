...
# assigns estimNames correspondence between R Names and e

# isolate DVs
var_table<-list_RDBES_Variables[list_RDBES_Variables$Table=="TE",]

dv_vars<-var_table[grepl(var_table$Required, pat="DV"),"Field.Name"]

id_vars<-var_table[grepl(var_table$Field.Name, pat="id"),"Field.Name"]

vars_keep<-c(id_vars, dv_vars)

colnames(y)[colnames(y) %in% dv_vars]<-var_table$estimName[match(colnames(y)[colnames(y) %in% dv_vars], var_table$Field.Name)]
